...
for x in ['J=(0,W.useRef)(null)','K=(0,W.useRef)(false)','e.key==="Shift"','shift||K.current','onDelete:()=>B.size>1?R():B.has(o.id)?R():e(o.id)','Delete selected (${B.size})']:
...
tracker={'version':1,'startingBalance':100,'transactions':[],'bets':[bet(i) for i in range(1,7)],'ladder':{'startStake':10,'targetReturn':100,'days':7,'results':['Pending']*7}}
  ...
  setTimeout(()=>{{
    ...
    setTimeout(()=>{{
      ...
      setTimeout(()=>{{
        ...
        const bulk=[...document.querySelectorAll('button')].find(b=>b.textContent.trim()==='Delete selected (5)');
        const rows=[...document.querySelectorAll('table.bet-table tbody tr')];
        const rowDelete=[...rows[2].querySelectorAll('button')].find(b=>b.textContent.trim()==='Delete');
        rowDelete?.click();
        setTimeout(()=>{{
          ...
          const ok=before===6&&boxes.length===6&&pattern==='111110'&&!!bulk&&!!rowDelete&&after===1&&stored===1;
...
with tempfile.TemporaryDirectory() as td:
    ...
    try:
        time.sleep(.3)
        r=subprocess.run([chrome,'--headless','--disable-gpu','--no-sandbox','--disable-background-networking','--virtual-time-budget=2900','--dump-dom','http://127.0.0.1:8767/index.html'],capture_output=True,text=True,timeout=35)
...
print('Shift-select browser QA passed: physical Shift keydown + fifth checkbox selects rows 1-5; deleting from a selected row removes the full five-bet selection')