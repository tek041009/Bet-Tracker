...
setTimeout(()=>{{[...document.querySelectorAll('aside nav button')].find(b=>b.textContent.includes('Bet Tracker'))?.click();setTimeout(()=>{{
 ...
 setTimeout(()=>{{
  ...
  setTimeout(()=>{{
   ...
   if(sels?.[0]){{sels[0].value='market';sels[0].dispatchEvent(new Event('change',{{bubbles:true}}));}}
   if(inp){{inp.value='Match Odds';inp.dispatchEvent(new Event('input',{{bubbles:true}}));inp.dispatchEvent(new Event('change',{{bubbles:true}}));}}
   setTimeout(()=>{{const rows=document.querySelectorAll('table.bet-table tbody tr');document.body.dataset.qa=(JSON.stringify(labels)===JSON.stringify(['Bookmaker','Sport','Source','Result'])&&!!document.querySelector('.bt-advanced-panel')&&rows.length===1&&rows[0].textContent.includes('West Ham'))?'pass':'fail';document.body.dataset.detail=labels.join('|')+' / '+rows.length+' / '+(rows[0]?.textContent||'');}},250);
  }},100);
 }},100);
}},100)}},80);
</script></body></html>'''
with tempfile.TemporaryDirectory() as td:
    ...
    try:
        time.sleep(.25);r=subprocess.run([chrome,'--headless','--disable-gpu','--no-sandbox','--disable-background-networking','--virtual-time-budget=2200','--dump-dom','http://127.0.0.1:8766/index.html'],capture_output=True,text=True,timeout=35)
