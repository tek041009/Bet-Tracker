...
if 'for(let t=0;t<75;t++){if(!form.isConnected)break;await wait(40)}' in html:
    raise SystemExit('Batch reliability QA: old 3 second save timeout still present')
...
# Verify the progress layer accepts and reports skipped already-saved bets.
...
print('Batch reliability QA passed: no fixed 120ms navigation race, tracker/add-form waits are 8s, saves are validation-aware, and retries skip already-saved Bet365 IDs')
