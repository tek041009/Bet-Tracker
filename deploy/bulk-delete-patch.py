...
('      T=v.reduce((o,r)=>o+_n(r),0),E=v.reduce((o,r)=>o+(r.profitLoss??0),0),S=v.filter(o=>o.result==="Win").length,y=v.filter(o=>o.result==="Loss").length;\n  return(0,c.jsxs)',
 '      T=v.reduce((o,r)=>o+_n(r),0),E=v.reduce((o,r)=>o+(r.profitLoss??0),0),S=v.filter(o=>o.result==="Win").length,y=v.filter(o=>o.result==="Loss").length;\n  (0,W.useEffect)(()=>{C(x=>{let z=new Set([...x].filter(id=>t.bets.some(p=>p.id===id)));return z.size===x.size?x:z})},[t.bets]);\n  let A=v.length>0&&v.every(o=>B.has(o.id)),D=(id,checked,shift)=>{C(x=>{let z=new Set(x),anchor=r1.__btLastSelectedId;if(shift&&anchor){let a=v.findIndex(o=>o.id===anchor),b=v.findIndex(o=>o.id===id);if(a>=0&&b>=0){let lo=Math.min(a,b),hi=Math.max(a,b);v.slice(lo,hi+1).forEach(o=>checked?z.add(o.id):z.delete(o.id))}else checked?z.add(id):z.delete(id)}else checked?z.add(id):z.delete(id);return z});r1.__btLastSelectedId=id;r1.__btMouseShift=false},H=()=>{r1.__btLastSelectedId=null;r1.__btMouseShift=false;C(x=>{let z=new Set(x);if(A)v.forEach(o=>z.delete(o.id));else v.forEach(o=>z.add(o.id));return z})},R=()=>{if(B.size&&q?.([...B])){C(new Set);r1.__btLastSelectedId=null;r1.__btMouseShift=false}};\n  return(0,c.jsxs)'),
...
('(0,c.jsx)("tbody",{children:v.map(o=>(0,c.jsx)(m1,{item:o,onEdit:()=>a(o),onDelete:()=>e(o.id)},o.id))})',
 '(0,c.jsx)("tbody",{children:v.map(o=>(0,c.jsx)(m1,{item:o,onEdit:()=>a(o),onDelete:()=>B.has(o.id)?R():e(o.id),isSelected:B.has(o.id),onSelect:(checked,shift)=>D(o.id,checked,shift)},o.id))})'),
...
('(0,c.jsx)("td",{children:(0,c.jsxs)("div",{className:"row-actions",children:[t.receiptImage&&',
 '(0,c.jsx)("td",{children:(0,c.jsxs)("div",{className:"row-actions",children:[(0,c.jsx)("input",{type:"checkbox",className:"bt-row-select",checked:o,onMouseDown:e=>{r1.__btMouseShift=!!e.shiftKey},onChange:e=>r(e.currentTarget.checked,!!(e.nativeEvent?.shiftKey||r1.__btMouseShift)),"aria-label":`Select ${e}`,title:"Select bet · Shift-click another checkbox to select the whole range"}),t.receiptImage&&'),
...
print('Applied Bet Tracker row selection, robust Shift-click range selection, selected-row bulk delete, select-all-filtered and single-confirm bulk delete')
