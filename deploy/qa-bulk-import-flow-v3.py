...
required=['onBulkDelete:q','className:"bt-row-select"','aria-label":"Select all filtered bets"','Delete selected (${B.size})','onBulkDelete:B','id="bt-bulk-delete-styles"','id="bt-import-progress-styles"','Adding your bets to Bet Tracker','window.__btPaddyImportProgress?.({added,total})','return await importSelected(modal);','Import complete. Opening Bet Tracker…','goToBetTracker();await sleep(650);screen.remove()']
for x in required:
    if x not in html: raise SystemExit(f'QA v3 failed: missing {x}')
...
if not all([main_m,text_m,review_m,review_css,progress_css]): raise SystemExit('QA v3 failed: scripts/styles missing')
...
if actual_wrapper not in text_js: raise SystemExit('QA v3 failed: real reviewed importer does not return import result')
if text_js.count('window.__btPaddyImportProgress?.({added,total})')<2: raise SystemExit('QA v3 failed: real importer does not publish start/per-bet progress')
for name,js in [('main',main_js),('text',text_js),('review',review_js)]:
    ...
    if r.returncode: raise SystemExit(f'QA v3 syntax failed {name}: {r.stderr}')
chrome=shutil.which('google-chrome') or shutil.which('chromium') or shutil.which('chromium-browser')
if not chrome: raise SystemExit('QA v3 failed: no Chrome')
tracker={'version':1,'startingBalance':100,'transactions':[],'bets':[{'id':'b1','date':'2026-09-14','app':'Paddy Power','sport':'Football','type':'Low Risk (Around Evs)','source':'My Pick','betFormat':'Single','odds':2,'stake':5,'potentialReturns':10,'bankrollRisk':.05,'result':'Win','profitLoss':5,'balanceAfter':105,'event':'A v B','market':'Match Odds','selection':'A','legs':[],'createdAt':1},{'id':'b2','date':'2026-09-14','app':'Paddy Power','sport':'Football','type':'Low Risk (Around Evs)','source':'My Pick','betFormat':'Single','odds':2,'stake':5,'potentialReturns':10,'bankrollRisk':.05,'result':'Loss','profitLoss':-5,'balanceAfter':100,'event':'C v D','market':'Match Odds','selection':'C','legs':[],'createdAt':2},{'id':'b3','date':'2026-09-15','app':'Bet365','sport':'Darts','type':'Medium Risk (Evs - 10/1)','source':'My Pick','betFormat':'Single','odds':3,'stake':5,'potentialReturns':15,'bankrollRisk':.05,'result':'Pending','profitLoss':None,'balanceAfter':100,'event':'E v F','market':'Match Betting','selection':'E','legs':[],'createdAt':3}],'ladder':{'startStake':10,'targetReturn':100,'days':7,'results':['Pending']*7}}
...
const wrap=document.createElement('div');wrap.className='bt-import-backdrop';wrap.innerHTML=`<div class="bt-import-modal" data-bt-import-mode="text"><div class="bt-import-actions"><button class="bt-use-import">Review</button></div><input class="bt-multi-check" data-i="0" type="checkbox" checked><input class="bt-multi-check" data-i="1" type="checkbox" checked></div>`;document.body.appendChild(wrap);const modal=wrap.querySelector('.bt-import-modal');const parsed=id=>({{_betId:id,date:'2026-09-15',betFormat:'Bet Builder',result:'Win',stake:5,returns:10,free:'',_receiptOdds:2,notes:'',source:'My Pick',_parseIssues:[],legs:[{{sport:'Football',event:'A v B',market:'Match Odds',selection:'A',odds:null,result:'Win',_superSubReplacement:''}}]}});modal.__btTextResults=[{{raw:'',parsed:parsed('1')}},{{raw:'',parsed:parsed('2')}}];window.__btImportReviewedPaddy=async(m,items)=>{{const total=items.length;window.__btPaddyImportProgress?.({{added:0,total}});for(let i=1;i<=total;i++){{await new Promise(r=>setTimeout(r,120));window.__btPaddyImportProgress?.({{added:i,total}})}}return {{added:total,total,error:null}}}};window.__btOpenPaddyReview(modal);setTimeout(()=>document.querySelector('.bt-pr-import')?.click(),30);setTimeout(()=>{{const o=document.querySelector('.bt-importing-overlay');document.body.dataset.mid=o?.querySelector('.bt-importing-count')?.textContent.trim()||'';document.body.dataset.status=o?.querySelector('.bt-importing-status')?.textContent.trim()||'';}},500);setTimeout(()=>{{const done=!document.querySelector('.bt-importing-overlay');document.body.dataset.qa=(document.body.dataset.mid==='2 / 2'&&document.body.dataset.status==='Import complete. Opening Bet Tracker…'&&done&&Number(document.body.dataset.nav||0)>=2)?'pass':'fail';}},1200);
</script></body></html>'''
with tempfile.TemporaryDirectory() as td:
    td=Path(td);(td/'bulk.html').write_text(bulk,encoding='utf-8');(td/'import.html').write_text(imp,encoding='utf-8');server=subprocess.Popen(['python3','-m','http.server','8765','--bind','127.0.0.1'],cwd=td,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    try:
        time.sleep(.3)
        for name,budget in [('bulk.html',2000),('import.html',1800)]:
            ...
            if r.returncode or 'data-qa="pass"' not in r.stdout:
                b=re.search(r'<body([^>]*)>',r.stdout,re.S);raise SystemExit(f'QA v3 browser failed {name}: '+(b.group(1) if b else (r.stdout+r.stderr)[-2500:]))
    finally:
        server.terminate();
        try:server.wait(timeout=2)
        except subprocess.TimeoutExpired:server.kill()
print('QA v3 passed: real tracker supports multi-select bulk delete; real importer wiring emits progress/returns result; loading UI reaches total and returns to Bet Tracker')
